...
checks={
 ...
 'cumulative module order preserves local before extension':order in man,
...
if failed: raise SystemExit('FAIL cumulative district dispatch contract: '+', '.join(failed))
print(f'PASS cumulative district dispatch contract: {len(checks)} checks')
