...
with sync_playwright() as pw:
    ...
    setup=page.evaluate('''()=>{
      ...
      if(!Game.ovCamera)throw Error('overworld camera was not initialized');
      ...
      return{src:src.id,contact:src.contact||null,offer,panelText:panel.textContent,width:rect.width,right:rect.right,scroll:document.documentElement.scrollWidth,inner:innerWidth,camera:!!Game.ovCamera};
    }''')
    assert setup['camera'] and setup['offer'] and setup['offer']['blocks']>=10,setup
    ...
    print('PASS city dispatch browser: initialized overworld lifecycle, physical source gate, dynamic route risk, real pendingPath, persistence, street-step delivery, rewards, 390x844 UI')
