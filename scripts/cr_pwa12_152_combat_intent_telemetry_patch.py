...
core=(ROOT/'src/legacy/core-v5.js').read_text(encoding='utf-8');gfx=(ROOT/'src/legacy/graphics-v6.js').read_text(encoding='utf-8')
checks={
 'fire_maneuver_preserved':'function enemyFireManeuverStateV146()' in ai and "intent==='SUP'" in ai and "intent==='GO'" in ai,
 'intent_dataset':"cue.dataset.intent=intent" in ai,'watch_class':"intent==='WATCH'" in ai,
 'all_mobile_codes':all((f"data-intent='{x}'" in css) for x in ('CONTACT','SCAN','PIN','FIN','SWEEP','BASE','WATCH','SUP','GO')),
 'pointer_passthrough':'pointer-events:none' in css,'compact_mobile':'width:24px' in css and 'height:10px' in css,
 ...
 'combat_core_dpr_cap':'const dpr = Math.min(window.devicePixelRatio || 1, 2);' in core,
 'overworld_core_uncapped':'const dpr = window.devicePixelRatio || 1;' in core,
 'combat_backdrop_dpr_cap':'dpr=Math.min(window.devicePixelRatio||1,2)' in gfx,
 'graphics_other_dpr_preserved':'dpr=window.devicePixelRatio||1' in gfx,
 'schema_neutral':'enemyFireManeuverV146' not in core,
}
print(json.dumps(checks,sort_keys=True));failed=[k for k,v in checks.items() if not v]
if failed: raise SystemExit('FAILED: '+', '.join(failed))
print('PWA12.152 combat intent telemetry contract PASS')
'''
NOTE=f'''# PWA12.152 Combat Intent Telemetry Candidate 01\n\nParent reconstruction: exact PWA12.145 Integration RC03 -> qualified PWA12.148 cumulative Heavy suppression/fire-and-maneuver rebase -> PWA12.151 tactical combat DPR cap -> this telemetry pass. Formal PWA12.104 canonical is untouched.\n\n## Player-facing change\nPhone/short-screen combat keeps the full semantic enemy intent in DOM text while rendering a bounded 24x10 non-interactive telemetry strip: CONTACT=CTC, SCAN=SCN, PIN, FIN, SWEEP=SWP, BASE=BAS, WATCH=WCH, SUP, GO. SUP/GO remain visibly distinct so the Heavy suppression lane and assault handoff read as one coordinated maneuver. Desktop retains the full intent words.\n\nThe overlay uses pointer-events:none and does not alter board/cell geometry, AP, initiative, LOS, target selection, damage, routing, suppression rules, world time, persistence, or save schema.\n\n## Preserved cumulative combat\nPWA12.148 Heavy suppression lane/arc planning/fire-and-maneuver remains authoritative. Heavy suppression costs 1 AP; hard cover counters lane interruption; assault handoff remains route preference, not free AP or teleportation. Tactical DPR is capped at 2 only in resizeCombatCanvas() and renderCombatBackdrop(); overworld DPR remains uncapped.\n\n## Version\n{VERSION}; schema remains 14.\n'''
...
def js_function_bounds(text,marker):
 start=text.index(marker); brace=text.index('{',start); depth=0;i=brace;quote=None;esc=False;line=False;block=False
 while i<len(text):
...
def patch_function(text,marker,old,new,label):
 a,b=js_function_bounds(text,marker);seg=text[a:b];count=seg.count(old)
 if count!=1:
  contexts=[];p=0
  while True:
   p=seg.find(old,p)
   if p<0:break
   contexts.append(seg[max(0,p-120):min(len(seg),p+len(old)+120)]) ;p+=len(old)
  raise RuntimeError(f'{label}: expected one scoped anchor, found {count}; contexts={contexts!r}')
 return text[:a]+seg.replace(old,new,1)+text[b:]

def main():
 if len(sys.argv)!=2: raise SystemExit('usage: patch.py CANDIDATE_ROOT')
 root=Path(sys.argv[1]).resolve();ai=root/'src/legacy/mercenary-intelligence-v8.js';s=ai.read_text(encoding='utf-8')
 if MARK in s: raise RuntimeError('telemetry patch already present')
 s=replace_once(s,OLD_CUE,NEW_CUE,'intent cue router');s=s.replace('// PWA12.146: bounded fire-and-maneuver handoff.','// PWA12.152 combat intent telemetry\n// PWA12.146: bounded fire-and-maneuver handoff.',1);ai.write_text(s,encoding='utf-8',newline='\n')
 cssp=root/'src/styles/24-v14-pwa12-5-fire-control.css';css=cssp.read_text(encoding='utf-8')
 ...
 corep=root/'src/legacy/core-v5.js';core=corep.read_text(encoding='utf-8');core=patch_function(core,'function resizeCombatCanvas','const dpr = window.devicePixelRatio || 1;','const dpr = Math.min(window.devicePixelRatio || 1, 2);','combat FX DPR');corep.write_text(core,encoding='utf-8',newline='\n')
 gfxp=root/'src/legacy/graphics-v6.js';gfx=gfxp.read_text(encoding='utf-8');gfx=patch_function(gfx,'function renderCombatBackdrop','dpr=window.devicePixelRatio||1','dpr=Math.min(window.devicePixelRatio||1,2)','combat backdrop DPR');gfxp.write_text(gfx,encoding='utf-8',newline='\n')
 (root/'tests/pwa12_152_combat_intent_telemetry_contract.py').write_text(CONTRACT,encoding='utf-8',newline='\n');(root/'DEVELOPMENT_NOTE_PWA12_152_COMBAT_INTENT_TELEMETRY.md').write_text(NOTE,encoding='utf-8',newline='\n')
 subprocess.run([sys.executable,'tools/rebundle_runtime.py'],cwd=root,check=True);subprocess.run([sys.executable,'tools/build_pwa11.py',VERSION],cwd=root,check=True);subprocess.run([sys.executable,'tests/pwa12_152_combat_intent_telemetry_contract.py'],cwd=root,check=True)
