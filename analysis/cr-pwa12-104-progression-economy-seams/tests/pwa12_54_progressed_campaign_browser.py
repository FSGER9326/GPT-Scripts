...
with sync_playwright() as pw:
 ...
 p.evaluate("""()=>{startNewGame('Persistence QA','Hacker','street');document.getElementById('story-modal')?.classList.remove('open');Game.credits=43210;Game.day=9;Game.hour=18.75;Game.rep=37;Game.heat={meridian:17,wraiths:4,iron:29,chrome:8,spine:13};Game.factionPower={meridian:61,wraiths:39,iron:52,chrome:47,spine:33};Game.districtControl={nero:'iron',undercity:'wraiths',steelworks:'iron',neon:'chrome',spine:'spine',mirage:'meridian'};Game.rival={name:'Black Halo',score:73,lastDay:8};Game.stash={weapons:['smg_mk1'],armor:['vest_mk1']};Game.safehouse={workshop:3,infirmary:2,bar:2,server:4};Game.missionHistory=[{id:'qa_done',name:'Ghost Wire',success:true,day:7}];Game.storyFlags={...Game.storyFlags,persistence_qa:true};Game.ovPlayer={x:11,y:7};const u=Game.roster[0];u.hp=Math.max(1,u.maxHp-9);u.xp=(u.xp||0)+777;u.level=Math.max(3,u.level||1);u.injuries=['burn'];u.loyalty=8;u.cyber=['reflex'];u.inventory={medkit:2,grenade:1};if(Game.cityLife)Game.cityLife.currentDistrict='steelworks';saveGame(1,false)}""")
 p.evaluate('()=>ChromeRequiemV14Boot.bridge.flush()')
 before=p.evaluate("()=>{const d=JSON.parse(serializeGame());delete d.savedAt;return d}")
 # Trash live state without touching stored slot, then load through v14 persistence bridge.
 p.evaluate("()=>{Game.credits=1;Game.day=1;Game.hour=0;Game.rep=0;Game.roster=[];Game.missionHistory=[];Game.storyFlags={};Game.ovPlayer={x:0,y:0}}")
 ok=p.evaluate('()=>ChromeRequiemV14Boot.runtime.load(1)');assert ok is True
 after=p.evaluate("()=>{const d=JSON.parse(serializeGame());delete d.savedAt;return d}")
 diffs=[k for k in sorted(set(before)|set(after)) if k!='version' and before.get(k)!=after.get(k)]
 assert not diffs,f'durable save/load mismatches: {diffs}'
 assert not errs,errs
 print('PASS PWA12.54 progressed campaign save/load equivalence across',len(before),'top-level durable fields')
 b.close()
